...
array of markets (condition IDs) to receive events for (for `user` channel)

assets\_ids

string\[\]

array of asset ids (token IDs) to receive events for (for `market` channel)
...
array of market ids (condition IDs) to receive events for (for `user` channel)
